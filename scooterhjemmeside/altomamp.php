...
$title = "amp scootere med elektrisk motor";
$overskrift = "amp";
...
'ampvelocity' => array(
     ...
     'specielt'            => array(
       'besked'            => ens('besked', '
producenter hedder måske Erato Manufacturing Co., Ltd.
eller Xiamen ZAP EV Manufacturing Co., Ltd.
Model : Erato 04 (1500w48v eller 2000w60v)
www.erato.cn
erato.cn.ec21.com
www.alibaba.com/member/pearland/product.html
www.erider.cn

Model : Erato 06 (1500w48v) (ligner en classic 50qt-9)
Model : Erato 03 (2000w60v) (ligner en retro 50qt-11)


EEC No. : E4*2002/24*0461*01

---

e4*2002/24*1955*00
tradename : Erider, Amgren (Arngren)
Type : B3000
B3000-00
B3000-01
Amgren Sku commuter AS (Norge)
Assembly plant : Xiamen ZAP EV Manufacturing Co., Ltd. (Xiamen City)
Check rdw.nl
commicial name : 
', '', '', '')

   ),'generelt'            => array(
       'model'             => ens('model', 'amp velocity', '', '', '')
      ,'ogsåkaldet'        => ens('ogsåkaldet', '"erider XM 3500li" (i følge bruger)', 'måske : city 4.0, x-treme XM3500, x-treme XM4000 (hedder x-treme i amerika)', '', '')
      ,'scootertype'       => ens('scootertype', '', '', '', '')
      ,'varianter'         => ens('varianter', 'fås med silicone eller lithium ion batterier', '', '', '')
      ,'pris'              => ens('pris', '11100', '15900', '', 'afhængig af batteri typen')
      ...
      ,'batterispec2'      => ens('batterispec2', '', '', 'lithium ion (2,6 KWh)', '')
      ...
      ,'oplader'           => ens('oplader', '', '', '', '')
      ,'ladetid'           => ens('ladetid', '', '', '', '')
      ,'rækkevidde'        => ens('rækkevidde', '', '80', '', 'med silicone batterier') . '<br>' . ens('rækkevidde', '', '110', '', 'med lithium ion batterier')
      ...
      ,'reduktionsratio'   => ens('reduktionsratio', '', '', '', '')
       ...
       'stel'              => ens('stel', '', '', '', '')
      ,'stelnummer'        => ens('stelnummer', 'LRZTD01X792', '', '', '')
      ...
      ,'bakgear'           => ens('bakgear', '', '', '', '')
      ,'importør'          => ens('importør', 'AMP Electro', '', '', '')
     ...
     'specielt'            => array(
       'besked'            => ens('besked', '
rider.cn siger :
Motor both 2000W power at 48V battery and 3000W power at 60V battery

Both silicon battery and LFP lithium battery are aviable on EEC paper

EEC on Moped class (L1e) , e4*2002/24*1954*00 for 45km/H speed 
', '', '', '')

   ),'generelt'            => array(
       'model'             => ens('model', 'amp ørsted', '', '', '')
      ,'ogsåkaldet'        => ens('ogsåkaldet', 'måske : retro 2.0, x-treme XM3100', 'beauty', '', '')
      ,'scootertype'       => ens('scootertype', '', '', '', '')
      ,'varianter'         => ens('varianter', 'fås med silicone eller lithium ion batterier', '', '', '')
      ,'pris'              => ens('pris', '7900', '11100', '', 'afhængig af batteri typen')
      ...
      ,'venderadius'       => ens('venderadius', '', '', '', '')
       ...
       'starterbatteri'    => ens('starterbatteri', '', '', '', '')
      ,'batterispec1'      => ens('batterispec1', '', '', 'silicone', '')
      ,'batterispec2'      => ens('batterispec2', '', '', 'lithium ion (1,92 KWh)', '')
      ,'batterivægt'       => ens('batterivægt', '', '', '', '')
      ,'batterilevetid'    => ens('batterilevetid', '', '', '', '')
      ,'generator'         => ens('generator', '', '', '', '')
      ,'sikring'           => ens('sikring', '', '', '', '')
      ...
      ,'oplader'           => ens('oplader', '', '', '', '')
      ,'ladetid'           => ens('ladetid', '', '', '', '')
      ,'rækkevidde'        => ens('rækkevidde', '', '50', '', 'med silicone batterier') . '<br>' . ens('rækkevidde', '', '80', '', 'med lithium ion batterier')
      ,'controller'        => ens('controller', '', '', '', '')
...
'download' => array(
    'erider / arngren typegodkendelse'      => 'eridertypegodkendelse.jpg'
