...
$title = "solex scootere med elektrisk motor";
$overskrift = "solex";
...
'esolex' => array(
     ...
     'specielt'            => array(
       'besked'            => ens('besked', '
Skrev til e-solex.dk (som var en hjemmeside der nu er fjernet) og fik dette svar.
'.citat('Vi er ved at lukke firmaet ned - derfor virker siden ikke. Der var desværre ikke den fornødne efterspørgsel efter den elektriske Solex i Danmark.').'

'.ahref('http://www.solexworld.fr/en/products/e-solex/').'
'.ahref('http://www.scootergalleri.dk/alb_vis.aspx?AlbumId=78760').'
', '', '', '')

   ),'generelt'            => array(
       'model'             => ens('model', 'e-solex', '', '', '')
      ,'ogsåkaldet'        => ens('ogsåkaldet', '', '', '', '')
      ,'scootertype'       => ens('scootertype', 'ES-01', '', '', '')
      ,'varianter'         => ens('varianter', '', '', '', '')
      ,'pris'              => ens('pris', '14995', '', '', '')
      ,'hastighed'         => ens('hastighed', '35', '', '', '(ifølge e-solex.dk)') . '<br>' . ens('hastighed', '25 eller 35', '', '', '(ifølge solexworld.fr)')
      ,'farver'            => ens('farver', 'sort-rød sort-sort sort-grå sort-brun', '', '', '')
      ,'forhandlere'       => ens('forhandlere', '', '', '', '')

   ),'motor'               => array(
       'motormodel'        => ens('motormodel', '', '36 volt', '400 watt', '')
      ,'motortype'         => ens('motortype', 'elektrisk', 'i baghjuls nav', 'børsteløs', '')
      ...
      ,'sædehøjde'         => ens('sædehøjde', '800-950', '', '', '(sadlen kan justeres i højden)')
      ...
      ,'bremserbag'        => ens('bremser', 'skive', '160', '', '')
      ,'eger'              => ens('eger', '', '', '')
      ...
      ,'venderadius'       => ens('venderadius', '', '', '', '')
       ...
       'starterbatteri'    => ens('starterbatteri', '37', '16', 'Lithium-Polymer (Lithium-Ion Polymer)', 'aftageligt (så det for eksempel kan oplades i lejlighed)')
      ,'batterispec1'      => ens('batterispec1', '', '', '', '')
      ,'batterispec2'      => ens('batterispec2', '', '', '', '')
      ,'batterivægt'       => ens('batterivægt', '3,9', '', '', '')
      ,'batterilevetid'    => ens('batterilevetid', '', '500', 'cirka 20.000', '')
      ...
      ,'oplader'           => ens('oplader', '', '', '', '')
      ,'ladetid'           => ens('ladetid', '4', '70%', '', '') . '<br>' . ens('ladetid', '8', '100%', '', '')
      ,'rækkevidde'        => ens('rækkevidde', '', 'op til 40', '', 'afhængig af motorens "power mode", førens vægt, vejtypen, dæktryk og vindretning (ifølge pdf fil fra solexworld.fr)')
      ...
      ,'kølervæske'        => ens('kølervæske', '', '', '', '')
       ...
       'transmissiontype'  => ens('transmissiontype', 'kæde', 'til pedalerne', '', '')
      ,'kobling'           => ens('kobling', '', '', '', '')
      ...
      ,'reduktionsratio'   => ens('reduktionsratio', '', '', '', '')
       ...
       'stel'              => ens('stel', '', '', '', '')
      ,'stelnummer'        => ens('stelnummer', 'VREES01A091', '', '', '')
      ,'udstødning'        => ens('udstødning', '', '', '', '')
      ,'euroemission'      => ens('euroemission', '', '', '', '')
      ,'vægttør'           => ens('vægttør', '', '', '', '')
      ,'vægtkørerklar'     => ens('vægtkørerklar', '40', '', '', '(med batteri ifølge solexworld.fr)') . ens('vægtkørerklar', '43,9', '', '', '(med batteri ifølge e-solex.dk)')
      ,'vægttotal'         => ens('vægttotal', '', '', '', '')
      ,'bæreevne'          => ens('bæreevne', '', '', '', '')
      ,'luftfiltertype'    => ens('luftfiltertype', '', '', '', '')
      ,'stigeevne'         => ens('stigeevne', '', '', '', '')
      ,'medfølger'         => ens('medfølger', '1 batteri, 1 oplader, 2 side spejle, brugermanual, 2 nøgler', '', '', '')
      ...
      ,'andet'             => ens('andet', 'ekstra udstyr : blinklys, front kurv, topboks', '', '', '')
