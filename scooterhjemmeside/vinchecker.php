...
')
     ...
     Z = ?
')
           ,array('7 + 8', 'net brake horsepower',        '46 = 4-6 HK')
           ,array('9', '',        '0 = ?')
           ,array('11',    'plant location',       'G = Shandong province
6 = ?
B = ?')
           ,array('alle', '',        '
LAEAGZ4006B831737
...
')
      ...
      case 'UH9' :
         $noget = array(
            array(
            array('*',     '?',      '
UH9AC504T____RC1______ = Trilet AC50 4T
UH9TRLC50____RC1______ = Trilet LC 50
UH9T2000M____RC1______ = Trilet 2000M
')
           ,array('4-9',     'model',      '
...
')
   ...
   foreach($valuea as $valueb){
      ...
      foreach($valueb as $valuec){
         ...
         $valuec = trim($valuec);
         $indhold .= '<td style="text-align: left; vertical-align: top;">' . mb_ucfirst($valuec) . '</td>';
   ...
   $indhold .= '<table class="vincheckerekstrainformationomhastighed borderradius5px">';
      ...
      if(isset($valueb[1]) && $valueb[1] != ''){
         ...
         $indhold .= '<tr><td>25 km/t : </td><td>' . mb_ucfirst($valueb[1]) . '</td></tr>';

      }

      if(isset($valueb[2]) && $valueb[2] != ''){

         $indhold .= '<tr><td>30 km/t : </td><td>' . mb_ucfirst($valueb[2]) . '</td></tr>';

      }

      if(isset($valueb[3]) && $valueb[3] != ''){

         $indhold .= '<tr><td>45 km/t : </td><td>' . mb_ucfirst($valueb[3]) . '</td></tr>';
...
function vinlengde($data){
   ...
   if(count($data) == 17){
      ...
      if(count($data) == 5 || count($data) == 6){
         ...
         $indhold .= ' (Er det et registeringsnummer fra en nummerplade du har indtastet ? Hvis det er så kan du ikke indtaste det her, det er kun til VIN/stelnummer på 17 tegn)';
...
function fasdasdfafasdfdaf($data){
...
//if($GLOBALS['setup']['loggetind']){
   ...
   if(file_exists($filvin)){
      ...
      if(filesize($filvin)){
         ...
         $vinkode = visfilform(htmlentities(file_get_contents($filvin, NULL, NULL, 0, 1048576), null, 'UTF-8'));
         $vinkode = str_replace("\r\n", "\n", $vinkode); // midlertidig løsning så \r\n ikke bliver til <br>\r\n
...
LLMTCB2954L053104
   T scooter
    C luftkølet 4 takt
     B 50 cc
      29 design nummer
          L fabrik lokalitet (L = Linhai, Zhejiang, Kina)
   ...
   R retro scooter
    ...
    P 150 cc
     J lingyun motor
      ...
      D diamo
       9 9,38 HK
          A zhejian lingyun motorcycle co ltd
           model måned
           1 januar
           b november

LAN
   motorcykel type (mærke)
   P 2 hjul yamasaki
   T 2 hjul motofino
   X 2 hjul MF/YM scooter
   K 3 hjul yamasaki
...
L4S
   ...
   D lowspeed køretøj
    ...
    D 1 cylinder lodret inclined væskekølet
    H 1 cylinder horisontal væskekølet
    L flere cylindre appose eller inline
    M flere cylindre appose eller inline
    R flere cylindre V type luftkølet
    S flere cylindre V type væskekølet
    V flere cylindre opposed luftkølet
    W flere cylindre opposed væskekølet
     ...
     Z >750cc
      D overhead camshaft
      X underhead camshaft
       A elstarter, kæde transmission
       ...
       W elstarter/håndstarts, anden transmission
          1 gyshan yongkan zhejiang kina
          2 gyshan yongkan zhejiang kina
          6 jinhua zhejiang kina
          8 jinhua zhejiang kina
          9 yongkang zhejiang kina
-->
